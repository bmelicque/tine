...
impl<'src> Parser<'src> {
    ...
    fn eat(&mut self, tokens: &[Token]) -> Range<usize> {
    ...
    fn expect(&mut self, token: Token) -> Range<usize> {
        match self.tokens.peek() {
    ...
    fn expect_either(&mut self, tokens: &[Token]) {
        match self.tokens.peek() {
    ...
    fn recover_before(&mut self, tokens: &[Token], sync: &[Token]) -> Range<usize> {
        let mut range = self
            .tokens
            .peek()
            .map(|token| token.1.clone())
    ...
    fn recover_at(&mut self, token: &[Token]) -> Range<usize> {
    ...
    fn sync(&mut self, before: &[Token]) -> Range<usize> {
